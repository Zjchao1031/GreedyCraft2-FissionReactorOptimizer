    ...
    QTimer::singleShot(0, this, [this]() {
        QMessageBox::information(
            ...
            QString::fromUtf8(
                "请在使用本工具前仔细阅读以下注意事项：\n"
                "0.本工具包含两种功能：\n"
                "   普通结构生成:选择燃料单元的数量和燃料种类，输出可行的设计方案。\n"
                "   辐照结构生成:选择五种燃料，生成带辐照仓且运行速率最大的设计方案。\n"
                "1.中子源由非自启动燃料单元自动决定，生成方案统一显示为“任意中子源”。\n"
                "2.普通结构生成仅支持 1、2、4 个燃料单元。\n"
                "3.辐照结构生成固定为一个中心辐照仓和五个燃料单元。\n"
                "4.两个生成区域互相独立，每次点击一个区域内的生成方案按钮。\n"
                "5.在生成的结构内点击燃料单元，可以检查能否替换成其他燃料。\n"
                "6.使用过程产生任何问题请在贪婪2交流1、4群@Atopts.\n"));
    });
}
